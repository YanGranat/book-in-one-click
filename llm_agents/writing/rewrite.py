...
def _load_post_prompt() -> str:
    prompt_path = Path(__file__).resolve().parents[2] / "prompts" / "post" / "writing" / "post.md"
    return prompt_path.read_text(encoding="utf-8") if prompt_path.exists() else ""


def _load_rewrite_prompt() -> str:
    prompt_path = Path(__file__).resolve().parents[2] / "prompts" / "post" / "rewriting" / "rewrite_post.md"
    return prompt_path.read_text(encoding="utf-8") if prompt_path.exists() else ""
