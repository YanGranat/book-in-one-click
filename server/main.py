...
async def log_view_ui(log_id: int, _: bool = Depends(require_admin)):
    ...
    html = (
        ...
        "render(text);"
        "async function refresh(){try{const r=await fetch('/logs/'+LOG_ID,{cache:'no-store'});const j=await r.json();if(j&&j.content&&(j.content.length>0)){text=j.content;render(text);}else{document.getElementById('content').textContent='No content';}}catch(e){document.getElementById('content').textContent='Failed to load';}}refresh();"
