...
async def result_view_ui_id(res_id: int):
    ...
    html = (
        ...
        "let text=(document.getElementById('raw')?document.getElementById('raw').value:'');let showRaw=false;"
        "function wc(md){try{let s=String(md||'');s=s.replace(/```[\\s\\S]*?```/g,' ');s=s.replace(/`[^`]*`/g,' ');s=s.replace(/!\\[[^\\]]*\\]\\([^)]*\\)/g,' ');s=s.replace(/\\[[^\\]]*\\]\\([^)]*\\)/g,' ');s=s.replace(/<[^>]*>/g,' ');s=s.replace(/[#>*_`~\-]+/g,' ');const m=s.trim().match(/\\S+/g)||[];return m.length;}catch(_){return 0;}}"
        "function render(md){let html='';try{if(window.marked&&window.marked.setOptions){window.marked.setOptions({gfm:true,breaks:true});}html=(window.marked?window.marked.parse(md):'');}catch(e){html='';}if(!html||html.trim()===''){const safe=md.replace(/</g,'&lt;').replace(/>/g,'&gt;');html='<pre>'+safe+'</pre>';}document.getElementById('content').innerHTML=showRaw?('<pre>'+md.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</pre>'):html;}"
        "render(text);async function refresh(){try{const r=await fetch('/results/'+RES_ID,{cache:'no-store'});const j=await r.json();if(j&&(j.content||j.path)){if(j.content)text=j.content;const meta=document.getElementById('meta');if(meta){const words=wc(text);const provIn=(/^- provider:\s*(.*)$/m.exec(text)||[])[1]||j.provider||'?';const langIn=(/^- lang:\s*(.*)$/m.exec(text)||[])[1]||j.lang||'?';const topicIn=(/^- topic:\s*(.*)$/m.exec(text)||[])[1]||j.topic||'?';meta.textContent=`provider=${provIn} | lang=${langIn} | topic=${topicIn} | id=${RES_ID} | words=${words}`;}render(text);}}catch(_){}}refresh();"
        "document.getElementById('copy').onclick=async()=>{try{await navigator.clipboard.writeText(text);alert('Copied');}catch(_){}};"
        ...
        async with SessionLocal() as s:
            ...
            for r in rows:
                try:
                    ...
                    if jid > 0:
                        # Check if JobLog exists for this job_id
                        log_check = await s.execute(select(JobLog).where(JobLog.job_id == jid).limit(1))
                        has_log = log_check.scalar_one_or_none() is not None
                    
                    # Delete if no job_id OR no log found
                    if jid <= 0 or not has_log:
...
async def meme_result_view_ui_id(res_id: int):
    ...
    html = (
        ...
        "header a{color:var(--brand)}.spacer{flex:1}.toolbar button{background:#1b2230;border:1px solid var(--line);color:var(--text);padding:6px 10px;border-radius:8px;cursor:pointer;margin-left:8px}"
        "[data-theme='light'] .toolbar button{background:#eef2f6;border:1px solid #e5e9f0;color:#0f172a}"
        "main{padding:12px}#content{max-width:1000px;margin:0 auto;background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:12px}a{color:var(--brand)}"
        "#content ol,#content ul{margin:0;padding-left:1.25em;list-style-position:outside}#content ol li,#content ul li{line-height:1.35;margin-bottom:6px;white-space:pre-wrap}#content li p{margin:0}#content p{margin:4px 0}#content h1,#content h2,#content h3,#content h4,#content h5,#content h6{color:var(--brand)}"
...
async def telegram_webhook(secret: str, request: Request):
    ...
    try:
        ...
        # Ensure admin commands are registered (idempotent)
        try:
