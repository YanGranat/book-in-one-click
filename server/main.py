...
async def result_view_ui_id(res_id: int):
    ...
    html = (
        ...
        "render(text);async function refresh(){try{const r=await fetch('/results/'+RES_ID,{cache:'no-store'});const j=await r.json();if(j&&(j.content||j.path)){if(j.content)text=j.content;const meta=document.getElementById('meta');if(meta){const words=wc(text);meta.textContent=`provider=${j.provider||'?'} | lang=${j.lang||'?'} | topic=${j.topic||'?'} | id=${RES_ID} | words=${words}`;}render(text);}}catch(_){}}refresh();"
        "document.getElementById('copy').onclick=async()=>{try{await navigator.clipboard.writeText(text);alert('Copied');}catch(_){}};"
