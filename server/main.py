...
async def log_view_ui(log_id: int, _: bool = Depends(require_admin)):
    ...
    html = (
        ...
        "function escapeOutsideCode(md){const chunks=md.split('```');for(let i=0;i<chunks.length;i+=2){chunks[i]=chunks[i].replace(/</g,'&lt;').replace(/>/g,'&gt;');}return chunks.join('```');}"
        "function fenceXmlBlocks(md){const re=/<input>[\\s\\S]*?<\\/input>/gi;const chunks=md.split('```');for(let i=0;i<chunks.length;i+=2){chunks[i]=chunks[i].replace(re, m=>'```xml\n'+m+'\n```');}return chunks.join('```');}"
        "function extractMeta(md){const out={};const lines=md.split('\n');for(let i=0;i<Math.min(lines.length,200);i++){const line=lines[i].trim();if(line.startsWith('- provider:')){out.provider=line.split(':').slice(1).join(':').trim();}else if(line.startsWith('- lang:')){out.lang=line.split(':').slice(1).join(':').trim();}else if(line.startsWith('- topic:')){out.topic=line.split(':').slice(1).join(':').trim();}}return out;}"
        "let text=(document.getElementById('raw')?document.getElementById('raw').value:'');let showRaw=false;"
        "function render(md){const raw=(md||'');const prepped=fenceXmlBlocks(raw);const escaped=escapeOutsideCode(prepped);let html='';"
