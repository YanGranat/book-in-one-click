#!/usr/bin/env python3
"""
Interactive generator for a topic summary (structured conspectus).
Saves results to output/topic_summary/.
"""
import os
import sys
...
from pipelines.summary.pipeline import build_instructions as build_summary_instructions
...
def main() -> None:
	parser = argparse.ArgumentParser(description="Generate a structured topic summary")
	...
	parser.add_argument("--out", type=str, default="topic_summary", help="Output subdirectory")
	...
	print(f"\n🔄 Генерирую структурированный конспект по теме: '{topic}'")
	progress("agent:init")
	print("⏳ Это может занять 20–60 секунд...")

	try:
		agent = Agent(
			name="Topic Summary Writer",
			instructions=build_summary_instructions(topic, args.lang),
			model="gpt-4o",
		)
		...
		filename = f"summary_{safe_slug(topic)}_{timestamp}.md"
		filepath = output_dir / filename

		progress("io:save")
		save_markdown(filepath, title=topic, generator="OpenAI Agents SDK", pipeline="TopicSummary", content=content)
